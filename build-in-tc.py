#!/bin/env python
'''
This script triggers a taskcluster task to build minidump_stackwalk, waits
for it to finish, fetches the artifacts, uploads them to tooltool,
and updates the in-tree tooltool manifests.
'''
...
def spawn_task(queue, args):
    ...
    with open(local_file('task.json'), 'rb') as template:
        ...
        keys['task_deadline'] = (now + datetime.timedelta(hours=1)).isoformat() + 'Z'
...
def main():
    parser = argparse.ArgumentParser(description='Build and upload minidump_stackwalk binaries')
